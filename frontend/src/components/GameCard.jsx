  ...
  return (
    <div
      ...
      className={`relative transition-all duration-300 ease-out transform cursor-pointer
        ${isSelected ? '-translate-y-3 z-30' : 'hover:-translate-y-1 z-10'}
        ${item.status === 'dissolving' ? 'animate-dissolve pointer-events-none' : ''}
        ${item.status === 'flying' ? 'animate-fly-in pointer-events-none' : ''}
        ${!canSelect ? 'cursor-default' : ''}`}
    >
      <div
        className={`w-[3.3rem] h-[4.8rem] rounded-xl flex flex-col items-center justify-between p-1.5 border-2 shadow-xl transition-all duration-300
          ${item.isRevealed
      ...
      >
        {item.isRevealed ? (
          <>
            <div className={`self-start font-black text-[10px] leading-none ${item.card.suit.color}`}>{item.card.rank}</div>
            <div className="text-2xl leading-none">{item.card.suit.emoji}</div>
            <div className={`self-end font-black text-[10px] leading-none rotate-180 ${item.card.suit.color}`}>{item.card.rank}</div>
          </>
        ) : (
